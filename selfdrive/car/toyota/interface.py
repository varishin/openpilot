...
class CarInterface(CarInterfaceBase):
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), has_relay=False, car_fw=[]):  # pylint: disable=dangerous-default-value
    ...
    if candidate in [CAR.PRIUS, CAR.PRIUS_2019]:
      ...
      ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [[0.24], [0.04]]
      ...
      ret.safetyParam = 73
      ret.wheelbase = 2.70
      ret.steerRatio = 16.9   # guess work
      tire_stiffness_factor = 0.6371   # hand-tune
      ret.mass = 3115. * CV.LB_TO_KG + STD_CARGO_KG
      ret.steerActuatorDelay = 0.55
      if prius_pid:
        ret.lateralTuning.init('pid')
        ret.lateralTuning.pid.kpV, ret.lateralTuning.pid.kiV = [[0.45], [0.11]]
