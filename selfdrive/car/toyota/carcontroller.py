  ...
  def update(self, enabled, CS, frame, actuators, pcm_cancel_cmd, hud_alert,
    ...
    if (frame % 3 == 0 and CS.CP.openpilotLongitudinalControl) or (pcm_cancel_cmd and Ecu.fwdCamera in self.fake_ecus):
      ...
      if pcm_cancel_cmd and CS.CP.carFingerprint == CAR.LEXUS_IS:
        ...
        can_sends.append(create_accel_command(self.packer, apply_accel, pcm_cancel_cmd, self.standstill_req, lead, CS.out.distance))
      else:
        can_sends.append(create_accel_command(self.packer, 0, pcm_cancel_cmd, False, lead, CS.out.distance))
    ...
    if frame > 1000 and not (CS.CP.carFingerprint in TSS2_CAR or CS.CP.carFingerprint == CAR.CAMRY or CS.CP.carFingerprint == CAR.CAMRYH): # 10 seconds after start and not a tss2 car
